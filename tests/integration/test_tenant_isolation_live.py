...
async def test_cross_tenant_application_denied():
    async with AsyncClient(base_url=BASE, timeout=20.0) as client:
        ...
        ea, eb = f"a{_uid()}@example.com", f"b{_uid()}@example.com"
        password = "TestPass12chars!"

        ra = await client.post(
            "/api/v1/auth/register",
            json={"email": ea, "password": password, "full_name": "A", "tenant_id": ta},
        )
        rb = await client.post(
            "/api/v1/auth/register",
            json={"email": eb, "password": password, "full_name": "B", "tenant_id": tb},
        )
        ...
        create = await client.post(
            "/api/v1/applications",
            headers=ha,
            json={"name": f"app-{_uid()}", "environment": "production"},
        )
        ...
        assert obj_id not in ids, f"BOLA leak in list: {ids}"
